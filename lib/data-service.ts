...
export async function getCourses(params: CourseQueryParams = {}): Promise<CourseWithRelations[]> {
    ...
    const { limit, sort, categoryId, institutionId, level } = params;
    const cacheKey = `courses:${categoryId || 'all'}:${institutionId || 'all'}:${level || 'all'}:${limit || 'all'}:${sort || 'default'}`;
    ...
    if (sort === 'popular') {
        orderByClause = 'ORDER BY c.enrollCount DESC, c.createdAt DESC';
    } else if (sort === 'newest') {
