
import { query, queryOne } from '../lib/mysql-direct';
import * as fs from 'fs';
import * as dotenv from 'dotenv';
dotenv.config({ path: '.env.local' });

async function main() {
    try {
        let sql = "";
        const tableName = 'institution_guest_courses';

        // Get Create Table
        const createRow = await queryOne<any>(`SHOW CREATE TABLE ${tableName}`);
        if (createRow && createRow['Create Table']) {
            sql += createRow['Create Table'] + ";\n\n";
        } else {
            console.error("Could not get create table");
            return;
        }

        // Dump Data
        const rows = await query<any>(`SELECT * FROM ${tableName}`);
        for (const r of rows) {
            const keys = Object.keys(r);
            const values = Object.values(r).map(v => {
                if (v === null) return 'NULL';
                if (typeof v === 'string') return `'${v.replace(/'/g, "\\'")}'`; // basic escape
                if (v instanceof Date) return `'${v.toISOString().slice(0, 19).replace('T', ' ')}'`;
                return v;
            });

            sql += `INSERT INTO ${tableName} (${keys.join(',')}) VALUES (${values.join(',')});\n`;
        }

        fs.writeFileSync('guest_courses_dump.sql', sql);
        console.log('Dumped guest_courses_dump.sql');
    } catch (e) { console.error(e); }
    process.exit(0);
}
main();
